...
public class Prototype {
    ...
    private static class NumCQ {
        ...
        final BigInteger c, ch, cl;

        NumCQ(int q, long lch, long lcl) {
            ch = BigInteger.valueOf(lch);
            cl = BigInteger.valueOf(lcl);
            c = ch.shiftLeft(63).or(cl);
        }
    }
    ...
    static {
        ...
        p(-123, 0x5000000000000000L, 0x0000000000000000L); // 5^1
        p(-121, 0x6400000000000000L, 0x0000000000000000L); // 5^2
        p(-119, 0x7d00000000000000L, 0x0000000000000000L); // 5^3
        p(-116, 0x4e20000000000000L, 0x0000000000000000L); // 5^4
        p(-114, 0x61a8000000000000L, 0x0000000000000000L); // 5^5
        p(-112, 0x7a12000000000000L, 0x0000000000000000L); // 5^6
        p(-109, 0x4c4b400000000000L, 0x0000000000000000L); // 5^7
        p(-107, 0x5f5e100000000000L, 0x0000000000000000L); // 5^8
        p(-105, 0x7735940000000000L, 0x0000000000000000L); // 5^9
        p(-102, 0x4a817c8000000000L, 0x0000000000000000L); // 5^10
        p(-100, 0x5d21dba000000000L, 0x0000000000000000L); // 5^11
        p(-98, 0x746a528800000000L, 0x0000000000000000L); // 5^12
        p(-95, 0x48c2739500000000L, 0x0000000000000000L); // 5^13
        p(-93, 0x5af3107a40000000L, 0x0000000000000000L); // 5^14
        p(-91, 0x71afd498d0000000L, 0x0000000000000000L); // 5^15
        p(-88, 0x470de4df82000000L, 0x0000000000000000L); // 5^16
        p(-86, 0x58d15e1762800000L, 0x0000000000000000L); // 5^17
        p(-84, 0x6f05b59d3b200000L, 0x0000000000000000L); // 5^18
        p(-81, 0x4563918244f40000L, 0x0000000000000000L); // 5^19
        p(-79, 0x56bc75e2d6310000L, 0x0000000000000000L); // 5^20
        p(-77, 0x6c6b935b8bbd4000L, 0x0000000000000000L); // 5^21
        p(-74, 0x43c33c1937564800L, 0x0000000000000000L); // 5^22
        p(-72, 0x54b40b1f852bda00L, 0x0000000000000000L); // 5^23
        p(-70, 0x69e10de76676d080L, 0x0000000000000000L); // 5^24
        p(-67, 0x422ca8b0a00a4250L, 0x0000000000000000L); // 5^25
        p(-65, 0x52b7d2dcc80cd2e4L, 0x0000000000000000L); // 5^26
        p(-63, 0x6765c793fa10079dL, 0x0000000000000000L); // 5^27
        p(-60, 0x409f9cbc7c4a04c2L, 0x1000000000000000L); // 5^28
        p(-58, 0x50c783eb9b5c85f2L, 0x5400000000000000L); // 5^29
        p(-56, 0x64f964e68233a76fL, 0x2900000000000000L); // 5^30
        p(-54, 0x7e37be2022c0914bL, 0x1340000000000000L); // 5^31
        p(-51, 0x4ee2d6d415b85aceL, 0x7c08000000000000L); // 5^32
        p(-49, 0x629b8c891b267182L, 0x5b0a000000000000L); // 5^33
        p(-47, 0x7b426fab61f00de3L, 0x31cc800000000000L); // 5^34
        p(-44, 0x4d0985cb1d3608aeL, 0x0f1fd00000000000L); // 5^35
        p(-42, 0x604be73de4838ad9L, 0x52e7c40000000000L); // 5^36
        p(-40, 0x785ee10d5da46d90L, 0x07a1b50000000000L); // 5^37
        p(-37, 0x4b3b4ca85a86c47aL, 0x04c5112000000000L); // 5^38
        p(-35, 0x5e0a1fd271287598L, 0x45f6556800000000L); // 5^39
        p(-33, 0x758ca7c70d7292feL, 0x5773eac200000000L); // 5^40
        p(-30, 0x4977e8dc68679bdfL, 0x16a872b940000000L); // 5^41
        p(-28, 0x5bd5e313828182d6L, 0x7c528f6790000000L); // 5^42
        p(-26, 0x72cb5bd86321e38cL, 0x5b67334174000000L); // 5^43
        p(-23, 0x47bf19673df52e37L, 0x79208008e8800000L); // 5^44
        p(-21, 0x59aedfc10d7279c5L, 0x7768a00b22a00000L); // 5^45
        p(-19, 0x701a97b150cf1837L, 0x3542c80deb480000L); // 5^46
        p(-16, 0x46109eced2816f22L, 0x5149bd08b30d0000L); // 5^47
        p(-14, 0x5794c6828721caebL, 0x259c2c4adfd04000L); // 5^48
        p(-12, 0x6d79f82328ea3da6L, 0x0f03375d97c45000L); // 5^49
        p(-9, 0x446c3b15f9926687L, 0x6962029a7edab200L); // 5^50
        p(-7, 0x558749db77f70029L, 0x63ba83411e915e80L); // 5^51
        p(-5, 0x6ae91c5255f4c034L, 0x1ca924116635b620L); // 5^52
        p(-2, 0x42d1b1b375b8f820L, 0x51e9b68adfe191d4L); // 5^53
        p(0, 0x53861e2053273628L, 0x6664242d97d9f649L); // 5^54
    ...
    static BigDecimal toDecimal(double v) {
        ...
        int k = ord10pow2(q) - 1;
        int ord2alpha = q + ord2pow10(-k);
        assert 1 <= ord2alpha && ord2alpha <= 4; // 1 <= alpha < 10
        // p5.c is approximation of alpha with 126 - ord2alpha fractional bits
        ...
        if (s >= 100) {
            ...
            if (uin10 || win10) {
                assert !(uin10 && win10);
                return toBigDecimal(sgn, uin10 ? s10 : t10, k);
            }
        } else if (s < 10) {
    ...
    static BigDecimal casePower2(boolean sgn, int expf) {
        int q = expf == 0 ? -1074 : expf - 1075;
        BigInteger c = BigInteger.ONE.shiftLeft(52);
        BigInteger cb = c.shiftLeft(2);
        BigInteger cbl = cb.subtract(BigInteger.ONE);
        BigInteger cbr = cb.add(BigInteger.valueOf(2));

        int k = ord10pow2(q - 1) - 1;
        int ord2alpha = q - 1 + ord2pow10(-k);
        assert 1 <= ord2alpha && ord2alpha <= 4; // 1 <= alpha < 10
        ...
        if (s >= 100) {
            long s10 = s - s % 10;
            long t10 = s10 + 10;
            boolean uin10 = vnl <= s10 * 2;
            boolean win10 = t10 * 2 <= vnr;
            if (uin10 || win10) {
                if (!win10) {
