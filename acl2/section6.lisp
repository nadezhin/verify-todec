...
#|
(acl2::with-arith5-help
 (define shortest-form
   ((c integerp)
    (q integerp))
   :measure (abs (ifix c))
   :returns (mv (shortest-c integerp :rule-classes :type-prescription)
                (shortest-q integerp :rule-classes :type-prescription))
   (acl2::b*
    ((c (ifix c))
     (q (ifix q))
     ((when (= c 0)) (mv 0 0))
     ((unless (integerp (/ c (D)))) (mv c q)))
    (shortest-form (/ c (D)) (+ q 1)))
   ///
   (fty::deffixequiv shortest-form)
   (defrule shortest-term-c-not-divides
     (acl2::b*
      (((mv shortest-c ?shortest-q) (shortest-form c q)))
      (implies (not (= shortest-c 0))
               (not (integerp (* (/ (D)) shortest-c))))))
   (defrule D-value-shortest-form
     (acl2::b*
      (((mv shortest-c shortest-q) (shortest-form c q)))
      (equal (D-value shortest-c shortest-q) (D-value c q)))
     :enable D-value)
   (defrule shortest-form-unique
     (implies (= (D-value c1 q1) (D-value c2 q2))
              (equal (shortest-form c1 q1) (shortest-form c2 q2)))
     :rule-classes ()
     :use (:instance lemma2
                     (c1 (ifix c1))
                     (q1 (ifix q1))
     ...
     ((defrule lemma0
      ...
      (defrule lemma1
        (implies (and (integerp c)
                      ...
                      (integerp qn)
                      (<= q qn)
                      (integerp (* c (expt (D) (- q qn)))))
                 (equal (shortest-form c q)
                        (shortest-form (* c (expt (D) (- q qn))) qn)))
        :rule-classes ()
        :induct (shortest-form c q)
        :hints (("subgoal *1/3" :use (:instance lemma0
                                                (c (* c (expt (D) (- q qn))))
                                                (q (- (1- qn) q))))))
      (defrule lemma2
        ...
        :hints (("subgoal 2" :use (:instance lemma1
                ...
                ("subgoal 1" :use (:instance lemma1
                                             ...
                                             (q q2)
                                             (qn q1)))))))
   (defrule D-length-shortest-form
     (acl2::b*
      (((mv shortest-c ?shortest-q) (shortest-form c q)))
      (<= (D-length shortest-c) (D-length c)))
     :rule-classes :linear
     :enable D-length
     :prep-lemmas
     ((defrule lemma
