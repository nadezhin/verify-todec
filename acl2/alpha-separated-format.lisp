 ...
 (define alpha-separated-format-aux
   ...
   (acl2::b*
    ((q (ifix q))
     ...
     ((mv a1 b1) (alpha-separated-rat-search CbMax alpha-regular))
     (k-irregular (1- (ordD (* 3/4 ulp2))))
     (ulpD-irregular (expt (D) k-irregular))
     (alpha-irregular (/ (* 1/2 ulp2) ulpD-irregular))
     ((mv a2 b2) (alpha-separated-rat-search CbMax alpha-irregular))
     ((mv a3 b3) (alpha-separated-format-aux f (1- q))))
    (mv (min (min a1 a2) a3) (min (min b1 b2) b3)))
   ///
   ...
   (defrule alpha-separated-format-aux-correct
     (acl2::b*
      ...
      (implies (and (integerp qmax)
                    ...
                    (<= (Qmin f) q)
                    (<= q qmax))
               (and (alpha-separated alpha-regular Cbmax a b)
                    (alpha-separated alpha-irregular Cbmax a b))))
     :rule-classes ()
...
(defconst *hp-b* 17/131072)
(rule (and (= #fx1.0000000000000p-16 *hp-a*)
           (= #fx1.1000000000000p-13 *hp-b*)))
...
(defrule alpha-separated-hp-correct
 (acl2::b*
  ...
  (implies (and (integerp q)
                (<= (Qmin f) q)
                (<= q (Qmax f)))
           (and (alpha-separated alpha-regular Cbmax *hp-a* *hp-b*)
                (alpha-separated alpha-irregular Cbmax *hp-a* *hp-b*))))
 :rule-classes ()
 :enable hp
...
(defrule alpha-separated-sp-correct
 (acl2::b*
  ...
  (implies (and (integerp q)
                (<= (Qmin f) q)
                (<= q (Qmax f)))
           (and (alpha-separated alpha-regular Cbmax *sp-a* *sp-b*)
                (alpha-separated alpha-irregular Cbmax *sp-a* *sp-b*))))
 :rule-classes ()
 :enable sp
...
(defrule alpha-separated-dp-correct
 (acl2::b*
  ...
  (implies (and (integerp q)
                (<= (Qmin f) q)
                (<= q (Qmax f)))
           (and (alpha-separated alpha-regular Cbmax *dp-a* *dp-b*)
                (alpha-separated alpha-irregular Cbmax *dp-a* *dp-b*))))
 :rule-classes ()
 :enable dp
